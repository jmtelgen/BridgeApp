...
import { loadDDS } from './dds-loader';
...
// Use solver's encoding: (3 - suit) * 13 + rank
export function cardToIndex(cardString: string): number {
  ...
  // Use solver's encoding: (3 - suit) * 13 + rank
  // This matches: dds_suit[0] = 3 - (rb_card / 13)
  return (3 - suitIndex) * 13 + rankIndex;
}
...
export function bufferToSolutions(instance: DDSInstance, outputArray: number, numCards: number): DDSSolution[] {
  ...
  for (let i = 0; i < numCards * 2; i += 2) {
    ...
    // Solver's encoding: (3 - suit) * 13 + rank
    // So to decode: suit = 3 - (cardValue / 13), rank = cardValue % 13
    const suit = 3 - Math.floor(cardValue / 13);
...
export async function solveBoard(params: {
  ...
  try {
    ...
    try {
      ...
      if (params.robot) {
        return result.filter((_, index) => index === result.length - 1);
      }
      return result;
